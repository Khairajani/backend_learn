{% macro on_run_end() %}
  {% if execute %}
    ...
    {% set jwt_token = var('openmetadata_jwt_token', none) %}
    {% set host_port = var('openmetadata_host_port', none) %}
    {% set service_name = var('openmetadata_service_name', 'dbt') %}
    ...
    {% else %}
      ...
      {% do log("   openmetadata_jwt_token: 'your-jwt-token'", info=True) %}
      {% do log("   openmetadata_host_port: 'http://localhost:8585/api'", info=True) %}
      {% do log("   openmetadata_service_name: 'your-service-name'", info=True) %}
    {% endif %}
